...
export class SupermarketRoom extends Room<SupermarketState> {
  ...
  onCreate(_options: any) {
    ...
    this.onMessage("dropBox", (client) => {
      const player = this.state.players.get(client.sessionId);
      if (!player || !player.holdingBoxId) return;
      const box = this.state.deliveryBoxes.get(player.holdingBoxId);
      if (box) {
        box.isHeld = false;
    ...
    this.onMessage("hireEmployee", (client) => {
      if (this.state.budget < 1500) return;
      ...
      this.state.employees.set(id, emp); this.state.budget -= 1500;
    ...
    this.onMessage("refillItem", (client, data: { id: string }) => {
      const player = this.state.players.get(client.sessionId);
      const item = this.state.placedItems.get(data.id);
      if (!player || !item || !item.type.startsWith("shelf_") || !item.productId || item.stock >= item.maxStock) return;

      // Priority 1: Use carried box if it matches
      if (player.holdingBoxId) {
        const box = this.state.deliveryBoxes.get(player.holdingBoxId);
        if (box && box.productId === item.productId) {
          ...
          this.broadcast("moneyChange", { amount: 0, type: "income" }); // Trigger visual feedback for interaction
          return;
        }
      }

      // Priority 2: Use general inventory
      const invStock = this.state.inventory.get(item.productId) || 0;
      if (invStock <= 0) return;
      const toAdd = Math.min(item.maxStock - item.stock, invStock);
      item.stock += toAdd; this.state.inventory.set(item.productId, invStock - toAdd);
      this.state.storeExp += Math.round(toAdd * 10);
    });
        ...
        case "idle": {
          if (emp.task === "register") {
            ...
            if (registers.length > 0) {
              // Position behind the counter (left side, facing right)
            ...
            this.state.placedItems.forEach(item => { if (item.type.startsWith("shelf_") && item.productId && item.stock < item.maxStock && (this.state.inventory.get(item.productId) || 0) > 0) targetShelf = item; });
            if (targetShelf) {
              emp.targetX = targetShelf.gridX; emp.targetZ = targetShelf.gridZ + 0.8;
              emp.state = "walking"; this.calculatePathGeneric(emp);
            }
          }
          break;
        }
        case "walking": { if (this.followPath(emp, step)) { emp.state = "working"; emp.workTimer = 40; } break; }
        case "working": {
          emp.workTimer--;
          if (emp.task === "register") {
            // Face right (towards customer)
            emp.rotY = Math.PI / 2;
            let found = false;
            ...
            emp.workTimer = found ? 40 : 20;
              ...
              if (shelf && shelf.productId && shelf.stock < shelf.maxStock) {
                ...
                if (inv > 0) { const add = Math.min(shelf.maxStock - shelf.stock, inv); shelf.stock += add; this.state.inventory.set(shelf.productId, inv - add); }
              }
              emp.state = "idle";
            } else emp.state = "idle";
          }
  ...
  private processNPCShopPayment(npc: NPC) {
    ...
    npc.targetX = 0; npc.targetZ = 9.5; npc.state = "leaving"; npc.activeIcon = "👋";
  ...
  private tickNPCs() {
    ...
    if (this.state.npcs.size < maxNPCs && Math.random() < spawnProb) {
      ...
      npc.x = 0; npc.z = 9.5; npc.targetX = 0; npc.targetZ = 9.5; npc.state = "idle"; npc.activeIcon = "🛒";
    ...
    this.state.npcs.forEach((npc, id) => {
      switch (npc.state) {
        ...
        case "browsing_shelf": {
          ...
          if (npc.browseTimer <= 0) {
            ...
            } else { npc.targetX = 0; npc.targetZ = 9.5; npc.state = "leaving"; npc.activeIcon = "👋"; }
            this.calculatePathGeneric(npc);
          }
          break;
        }
        case "walking_to_register": { if (this.followPath(npc, step)) { npc.state = "waiting_to_pay"; npc.activeIcon = "💰"; npc.rotY = -Math.PI / 2; } break; }
      ...
      this.state.npcs.forEach((o, oid) => {
        ...
        if (d < 0.6 && d > 0.01) { npc.x += (dx/d) * 0.02; npc.z += (dz/d) * 0.02; }
      });
    });
  }
  ...
  private followPath(entity: any, step: number): boolean {
    if (entity.pathX.length > 0) {
      ...
      if (d > 0.1) { entity.x += (dx/d) * step; entity.z += (dz/d) * step; return false; }
  ...
  private tickPlayers() {
    ...
    this.state.players.forEach((p) => {
      ...
      if (p.pathX.length > 0) {
        const dx = p.pathX[0]! - p.x; const dz = p.pathZ[0]! - p.z;
        if (Math.sqrt(dx*dx + dz*dz) > 0.05) p.rotY = Math.atan2(dx, dz);
      } else {
        const dx = p.targetX - p.x; const dz = p.targetZ - p.z;
        if (Math.sqrt(dx*dx + dz*dz) > 0.1) p.rotY = Math.atan2(dx, dz);
      }
      this.state.placedItems.forEach((i) => {
        const dx = p.x - i.gridX; const dz = p.z - i.gridZ; const d = Math.sqrt(dx*dx + dz*dz);
        if (d < 0.9) { const o = 0.9 - d; p.x += (dx/d) * o; p.z += (dz/d) * o; }
      });
  ...
  private findAStarPath(sx: number, sz: number, ex: number, ez: number) {
    ...
    while (open.length > 0) {
      ...
      for (const n of [{ x: curr.x + 1, z: curr.z }, { x: curr.x - 1, z: curr.z }, { x: curr.x, z: curr.z + 1 }, { x: curr.x, z: curr.z - 1 }]) {
        if (Math.abs(n.x) > 9 || Math.abs(n.z) > 9 || closed.has(`${n.x},${n.z}`)) continue;
        let block = false;
        this.state.placedItems.forEach(i => { if (Math.round(i.gridX) === n.x && Math.round(i.gridZ) === n.z) block = true; });
        if (block && !(n.x === ex && n.z === ez)) continue;
        const g = curr.g + 1; const h = Math.abs(n.x - ex) + Math.abs(n.z - ez);
        const exist = open.find(o => o.x === n.x && o.z === n.z);
        if (exist) { if (g < exist.g) { exist.g = g; exist.p = curr; } }
        else open.push({ x: n.x, z: n.z, g, h, p: curr });
      }
    }
    return null;
  }
  ...
  private saveState() {
    try {
      ...
      this.state.inventory.forEach((v, k) => { (d.inventory as any)[k] = v; });
      this.state.placedItems.forEach((i) => { d.placedItems.push({ id: i.id, type: i.type, productId: i.productId, gridX: i.gridX, gridZ: i.gridZ, rotation: i.rotation, stock: i.stock, maxStock: i.maxStock } as never); });
      this.state.employees.forEach((e) => { d.employees.push({ id: e.id, name: e.name, color: e.color, salary: e.salary, task: e.task } as never); });
      this.state.deliveryBoxes.forEach((b) => { d.deliveryBoxes.push({ id: b.id, productId: b.productId, amount: b.amount, x: b.x, z: b.z } as never); });
  ...
  private loadState() {
    try {
      if (fs.existsSync(this.saveFilePath)) {
        ...
        this.state.budget = d.budget; this.state.storeName = d.storeName; this.state.storeLevel = d.storeLevel; this.state.storeExp = d.storeExp;
        if (d.inventory) Object.keys(d.inventory).forEach(k => this.state.inventory.set(k, d.inventory[k]));
        ...
        if (d.employees) d.employees.forEach((ed: any) => {
          const e = new Employee(); e.id = ed.id; e.name = ed.name; e.color = ed.color; e.salary = ed.salary; e.task = ed.task;
        ...
        if (d.deliveryBoxes) d.deliveryBoxes.forEach((bd: any) => {
          const b = new DeliveryBox(); b.id = bd.id; b.productId = bd.productId; b.amount = bd.amount; b.x = bd.x; b.z = bd.z;
